@extends('staff.layouts.app', ['title' => 'Product Type'])
@section('content')
              ...
              <input type="text" id="myInput" placeholder="Tìm theo tên">
                ...
                @foreach($product_types as $product_type)
                  <tr>
                    ...
                    <td>{!!$product_type->discription!!}</td>
                    <td class="text-center"><a href="{{ route('staff-edit-product-type',['id'=>$product_type->id])}}"><i class="ion-paintbrush" ></i></a>
                     <a href="{{ route('staff-delete-product-type',['id'=>$product_type->id])}}" data-method="DELETE" data-confirm="Bạn chắc chắn muốn xóa {{$product_type->product_type_name}}" class="delete ml-2"><i class="ion-ios-trash"></i></a>
                    </td>
                  </tr>
                @endforeach
                  </tbody>
